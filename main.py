...
class ExecutionLogger:
    ...
    def _format_api_request(self, agent: str, payload: dict[str, Any], redacted: dict[str, Any], call_id: int) -> str:
        ...
        tools_hash = self._hash_short(tools_sig) if tools_sig else "none"
        if tools_sig:
            if tools_hash not in self._tools_seen:
                self._tools_seen[tools_hash] = call_id
                tools_note = f"Tools: {tools_red} (hash=TL_{tools_hash})"
            else:
                tools_note = f"Tools: cached (hash=TL_{tools_hash}, first_seen=# {self._tools_seen[tools_hash]})"
        else:
    ...
    def parse(tool_call_dict: dict[str, Any]) -> 'ToolCall':
        ...
        parsers = {
            "click_screen_element": lambda d: ClickArgs(
                ...
                d.get("reasoning", ""),
                d.get("element_name", ""),
                Coordinate(float(d.get("position", [0, 0])[0]), float(d.get("position", [0, 0])[1]))
            ),
            ...
            "drag_screen_element": lambda d: DragArgs(
                ...
                d.get("reasoning", ""),
                d.get("element_name", ""),
                Coordinate(float(d.get("start", [0, 0])[0]), float(d.get("start", [0, 0])[1])),
                Coordinate(float(d.get("end", [0, 0])[0]), float(d.get("end", [0, 0])[1]))
            ),
...
EXECUTOR_TOOLS = (
    ...
    {"type": "function", "function": {"name": "click_screen_element", "description": "Click visible UI element with left mouse button",
     "parameters": {"type": "object", "properties": {
         "outcome_status": {"type": "string", "description": "PROGRESS if UI changed as expected, NO_CHANGE if UI looks the same, BLOCKED if tried multiple times with no progress", "enum": ["PROGRESS", "NO_CHANGE", "BLOCKED"]},
         "reasoning": {"type": "string", "description": "What you see in screenshot that led to this action and whether previous similar action worked"},
         ...
         "required": ["outcome_status", "reasoning", "element_name", "position"]}}},
    ...
    {"type": "function", "function": {"name": "drag_screen_element", "description": "Drag element from start position to end position",
     "parameters": {"type": "object", "properties": {
         ...
         "required": ["outcome_status", "reasoning", "element_name", "start", "end"]}}},
...
class AgentMemory:
    ...
    def _format_action_summary(self, rec: ActionRecord) -> str:
        if isinstance(rec.args, ClickArgs):
            return f"@[{rec.args.position.x:.0f},{rec.args.position.y:.0f}] {rec.args.element_name}"
        elif isinstance(rec.args, DragArgs):
            return f"from [{rec.args.start.x:.0f},{rec.args.start.y:.0f}] to [{rec.args.end.x:.0f},{rec.args.end.y:.0f}]"
...
def run_agent(state: AgentState) -> str:
    for iteration in range(CFG.max_steps):
        ...
        last = state.memory.get_last_action_for_validation()
        if prev_outcome_reported and last and (last.outcome or "").strip().upper() == "PENDING":
            last.outcome = prev_outcome_reported
            last.validated_on = state.turn
            if prev_reasoning_note:
                last.validation_note = prev_reasoning_note[:220]
            LOGGER.log(f"Validated Turn {last.turn} outcome => {prev_outcome_reported}")

        # 2) Decide if we should schedule a planning review based on validated outcomes (less aggressive).
        if prev_outcome_reported in ("BLOCKED", "NO_CHANGE"):
